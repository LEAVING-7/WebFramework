...
Context::Context(async::Reactor& r, async::MultiThreadExecutor& e, async::TcpStream& s, std::unique_ptr<ParamsType> p,
                 QueriesType&& queries)
    : mReactor(r), mExecutor(e), mStream(s), mParams(std::move(p)), mQueries(std::move(queries)), mResponse(),
      mGroups(), mGroupIndex(0), mIsAborted(false)
{
}
} // namespace wf